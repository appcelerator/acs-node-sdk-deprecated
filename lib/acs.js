...
var Ti = Titanium = require('./titanium/titanium');
var fs = require('fs');
...
function ACSSDK(key, secret, baseURL) {
	if (!secret) {
		this.appKey = key;
	} else {
		this.oauthKey = key;
...
ACSSDK.prototype.sendRequest = function(url, method, data, callback, useSecure) {
...
function createACSSDK(key, secret, baseURL) {
	return new ACSSDK(key, secret, baseURL);
};

exports.createACSSDK = createACSSDK;
...
function defineCloud(Cloud) {
    ...
    var ACS = (function defineNativeACS() {
        ...
        function ACSSession(key, secret, baseURL) {
            return createACSSDK(key, secret, baseURL);
        }
